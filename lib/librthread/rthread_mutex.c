...
pthread_mutex_trylock(pthread_mutex_t *mutexp)
{
	...
	mutex = *mutexp;
	_rthread_debug(5, "%p: mutex_trylock %p (%p)\n", pthread_self(),
	    (void *)mutex, (void *)mutex->owner);

	return (_rthread_mutex_trylock(mutex, 1, NULL));
}

static int
_rthread_mutex_timedlock(pthread_mutex_t *mutexp, const struct timespec *abs,
    int timed)
{
	pthread_t self = pthread_self();
	pthread_mutex_t mutex;
	...
	_rthread_debug(5, "%p: mutex_%slock %p (%p)\n", self,
	    timed ? "timed" : "", (void *)mutex, (void *)mutex->owner);

	error = _rthread_mutex_trylock(mutex, 0, abs);
	if (error != EBUSY)
		return (error);
...
{
	return (_rthread_mutex_timedlock(mutexp, abs, 1));
}

int
pthread_mutex_lock(pthread_mutex_t *mutexp)
{
	return (_rthread_mutex_timedlock(mutexp, NULL, 0));
}
